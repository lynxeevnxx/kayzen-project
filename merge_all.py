...
# Extract the header/states area from code_277
main_split = "  return ("
main_idx = code_277.find(main_split)
header_states = code_277[:main_idx]

# Replace state variables to add theme toggle, sessions
# Find position to insert state
...
# Replace react imports to include useEffect, Link
header_states_updated = header_states_updated.replace(
    'import { useState } from "react";',
    'import { useState, useEffect } from "react";\nimport Link from "next/link";'
)

# Replace the type Tab in page header
header_states_updated = header_states_updated.replace(
...
# Construct the body
# Header element structure with dynamic styles
...
# Extract the footer from code_277
footer_start = code_277.find("{/* ==================== FOOTER ==================== */}")
footer_code = code_277[footer_start:]

# Replace the quick links in the footer to delete mentor and bootcamp
...
# Let's adjust the blog block color system to adapt to isDarkMode toggle!
# The mockup image has a light theme by default for the blog tab, let's verify if we need it to dynamically adapt to dark/light theme.
# In the prompt: "mengapa warna tab blog putih sendiri, buat sistem darkmode dan light mode".
# Yes! The blog tab layout in code_277:
# isDarkMode ? "bg-brand-dark text-white" : "bg-[#FAFBFD] text-gray-800"
# This is perfect, it adapts dynamically to the toggle!

# Assemble all parts
final_page_code = f"""{header_states_updated}
{merged_navbar}
{beranda_block}
{program_block}
{kemitraan_block}
