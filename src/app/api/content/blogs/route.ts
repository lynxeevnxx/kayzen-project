...
export async function GET() {
  try {
    await initTables();
    const { data: blogs, error } = await getSupabase().from("blogs").select("id, title, slug, category, author, image, excerpt, content, created_at").eq("status", "published").order("created_at", { ascending: false });
