...
export default function DaftarPage() {
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Email Berhasil Diverifikasi! Akun Terbuat.");
      ...
      <div className="hidden lg:flex lg:col-span-6 bg-[#0c0e17] flex-col justify-between p-12 text-white relative overflow-hidden border-r border-white/5">
        ...
        <div className="space-y-8 relative z-10 text-left my-auto max-w-lg">
          ...
          <h2 className="font-display text-3xl font-extrabold leading-tight text-white">
            Pendaftaran Wajib Akun Terverifikasi.
          </h2>

          <div className="p-6 rounded-2xl glassmorphism border border-white/10 relative shadow-xl">
            <p className="text-xs text-white/90 leading-relaxed italic">
              &quot;Sistem verifikasi menjamin seluruh karya riset dan sertifikat terikat secara sah pada email terverifikasi Anda.&quot;
            </p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="space-y-2">
            <h1 className="font-display text-2xl sm:text-3xl font-extrabold tracking-tight text-gray-900">
              {step === "form" ? "Buat Akun Terverifikasi" : "Verifikasi Email Anda"}
            </h1>
            <p className="text-xs text-gray-500">
              {step === "form" 
                ? "Daftar dengan Akun Google atau verifikasi email valid Anda terlebih dahulu."
                : `Kode OTP verifikasi telah dikirim ke ${email}`}
            </p>
          </div>
          ...
          {step === "form" ? (
            <>
              {/* OPSI 1: LOGIN / DAFTAR GOOGLE ACCOUNT */}
              <button
              ...
              <div className="flex items-center gap-3 text-xs text-gray-400">
                ...
                <span>atau verifikasi via email valid</span>
                <span className="h-[1px] bg-gray-200 flex-grow" />
              </div>

              {/* OPSI 2: FORM DAFTAR DENGAN VERIFIKASI EMAIL */}
              <form onSubmit={handleSendOTP} className="space-y-4">
                ...
                >
                  ...
                  Kirim Kode Verifikasi Email (OTP)
                </button>
              </form>
            </>
          ) : (
          ...
          <div className="text-center pt-4 text-xs text-gray-500">
            Sudah memiliki akun?{" "}
            <Link href="/masuk" className="font-bold text-brand-purple hover:underline">
              Masuk ke akun Anda
            </Link>
          </div>
        </div>
